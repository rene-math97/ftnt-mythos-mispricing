/* ═══════════════════════════════════════════════
   FTNT THESIS — APP JS
   DCF interactivity, theme toggle, animations
════════════════════════════════════════════════ */
...
const WACC       = 0.09;
const TV_MULT    = 28;
const BASE_REV   = 6_799_600_000;
const BASE_FCF_M = 0.368;
const SHARES     = 743_648_102;
const NET_CASH   = 2_592_500_000;
const CUR_PRICE  = 82.53;
const CONV       = 0.40;

const SCENARIOS = {
  bull: {
    label:    'Bull Case',
    desc:     '18% Revenue CAGR · FCF margin expanding to 42%',
    cagr:     0.18,
    fcfEnd:   0.42,
    color:    'bull',
  ...
  base: {
    label:    'Base Case',
    desc:     '14% Revenue CAGR · FCF margin stable at 37%',
    cagr:     0.14,
    fcfEnd:   0.37,
    color:    'base',
  ...
  bear: {
    label:    'Bear Case',
    desc:     '10% Revenue CAGR · FCF margin compressing to 33%',
    cagr:     0.10,
    fcfEnd:   0.33,
    color:    'bear',
...
function calcDCF(sc) {
  ...
  for (let yr = 1; yr <= 5; yr++) {
    ...
    const margin = BASE_FCF_M + (sc.fcfEnd - BASE_FCF_M) * (yr / 5);
    const fcf    = rev * margin;
    const pv     = fcf / Math.pow(1 + WACC, yr);
    revs.push(rev); fcfs.push(fcf); pvs.push(pv);
    pvSum += pv;
  }
  const tv        = TV_MULT * fcfs[4];
  const pvTV      = tv / Math.pow(1 + WACC, 5);
  const ev        = pvSum + pvTV;
  const intrinsic = (ev + NET_CASH) / SHARES;
  const target12m = CUR_PRICE + CONV * (intrinsic - CUR_PRICE);
  return { revs, fcfs, pvs, pvSum, tv, pvTV, ev, intrinsic, target12m };
}
...
function renderScenario(key) {
  ...
  // Price bar (scale: $60 to max of $220)
  const MIN  = 60;
  const MAX  = Math.max(220, Math.ceil(data.intrinsic / 10) * 10 + 20);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const sections = ['hero','summary','dcf','comps','why-not','catalysts','risks','recommendation'];
